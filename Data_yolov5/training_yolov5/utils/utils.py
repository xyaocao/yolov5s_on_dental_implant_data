...
def build_targets(p, targets, model):
    # Define the device based on the model's device
    ...
    # Ensure targets are on the same device as the model
    gain = torch.ones(6, device=device)  # normalized to gridspace gain
    targets = targets.to(device)  # Move targets to the model device
    
    # Device for offsets and anchors
    ...
    for i in range(det.nl):
        anchors = det.anchors[i].to(device)  # Ensure anchors are on the model's device
...
def output_to_target(output, width, height):
    ...
    """
    # Ensure the main output tensor is on the CPU
        ...
        if o is not None:
            for pred in o:
                if isinstance(pred, torch.Tensor):
                    pred = pred.cpu().numpy()  # Move each prediction to CPU
    ...
    # Convert targets to a NumPy array after ensuring all elements are on the CPU
    return np.array([[t if not isinstance(t, torch.Tensor) else t.cpu().numpy() for t in target] for target in targets])
